...
const Anecdote = ({ anecdotes }) => {
  ...
  const [anecdote] = anecdotes.filter(el => el.id == Number(anecdoteId))
  return (
    <div>
...
const CreateNew = props => {
  ...
  const handleSubmit = e => {
    e.preventDefault()
    props.addNew({
      content: content.value, author: author.value, info: info.value, vote: 0
    })
    // setContent('')
    // setAuthor('')
    // setInfo('')
  ...
  return (
    <Router>
      <div>
        ...
        <Link style={padding} to='/anecdotes:id'>
          create
        </Link>
      </div>
